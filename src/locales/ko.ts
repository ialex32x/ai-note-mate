export const ko: Record<string, string> = {
	...
	'settings.speechToTextConfig': '음성 텍스트 변환',
	'settings.speechToTextConfigDesc': '저장된 음성 텍스트 변환 설정을 선택하세요',
	'settings.addSpeechToTextConfig': '설정 추가',
	'settings.sttConfigName': '설정 이름',
	'settings.sttConfigNamePlaceholder': '이 설정의 이름을 입력하세요',
	'settings.deleteSpeechToTextConfigDesc': '이 설정을 삭제합니다. 최소 1개의 설정이 필요합니다',
	'settings.sttApiKeyDesc': '음성 텍스트 변환용 API 키',
	'settings.sttRegion': '리전',
	'settings.sttRegionDesc': 'DashScope 음성 텍스트 변환 서비스의 리전을 선택하세요',
	'settings.sttRegionCnBeijing': '중국 북부 2 (베이징)',
	'settings.sttRegionUsEast1': '미국 (버지니아)',
	'settings.sttRegionApSoutheast1': '싱가포르',
	'settings.sttRegionEuCentral1': '독일 (프랑크푸르트)',
	'settings.sttWorkspaceId': '워크스페이스 ID',
	'settings.sttWorkspaceIdDesc': 'DashScope 워크스페이스 ID. 싱가포르 및 프랑크푸르트 리전에서 필수입니다',
	'settings.sttShortModel': '짧은 오디오 모델',
	'settings.sttShortModelDesc': '짧은 오디오 실시간 변환에 사용되는 모델',
	'settings.sttLongModel': '긴 오디오 모델',
	'settings.sttLongModelDesc': '긴 오디오 비동기 파일 변환에 사용되는 모델',
	'settings.sttEmpty': '음성 텍스트 변환이 설정되지 않았습니다. 추가하여 오디오 트랜스크립션을 활성화하세요.',
	'settings.tcSecretId': 'SecretId',
	'settings.tcSecretIdDesc': 'Tencent Cloud SecretId. Tencent Cloud 콘솔에서 획득하세요',
	'settings.tcSecretKey': 'SecretKey',
	'settings.tcSecretKeyDesc': 'Tencent Cloud SecretKey. Tencent Cloud 콘솔에서 획득하세요',
	'settings.tcAsrRegion': 'ASR 리전',
	'settings.tcAsrRegionDesc': 'Tencent Cloud 음성 인식 서비스 리전 (예: ap-guangzhou, ap-shanghai)',
	'settings.tcEngineModelType': '엔진 모델',
	'settings.tcEngineModelTypeDesc': '음성 인식 엔진 유형. 16k_zh는 중국어, 8k_zh는 전화 통화용',
	'settings.tcCosBucket': 'COS 버킷 (선택 사항)',
	'settings.tcCosBucketDesc': 'COS 버킷 이름 (형식: name-appid). 5 MB 초과 파일에 필요',
	'settings.tcCosRegion': 'COS 리전',
	'settings.tcCosRegionDesc': 'COS 버킷 리전 (예: ap-guangzhou). 버킷 설정 시 필수',
	...
	'settings.agentMdTemplate': '## 규칙\n- 명확하고 간결한 언어를 사용하세요. Markdown 서식을 권장합니다.\n- 관련 노트를 언급할 때는 위키 링크(wiki-link)를 사용하세요.\n- 요청이 모호할 때는 명확히 하기 위한 질문을 하세요.\n\n## Mermaid 차트\n사용자의 요청에 시각화가 도움이 되는 데이터(비교, 비율, 추세, 워크플로우)가 포함된 경우 Mermaid 코드 블록을 출력하세요. 사용법:\n- `pie`: 비율 / 백분율\n- `xychart-beta`: 막대 차트 / 비교\n- `flowchart` 또는 `graph`: 프로세스 / 관계\n- `sequenceDiagram`: 상호작용 / 타임라인\n항상 구체적인 데이터를 우선 사용하고, 숫자를 지어내지 마세요.',
	...
	'settings.builtinCodeAgent': '내장 코드 에이전트',
	'settings.builtinCodeAgentDesc': '내장 코드 에이전트를 활성화하여 필요할 때 짧은 코드 조각을 실행할 수 있는 JavaScript 실행 도구를 어시스턴트에 제공합니다',
	'settings.followUpSection': 'Follow-up',
	'settings.followUpSuggestions': '후속 제안 버튼',
	'settings.followUpSuggestionsDesc': '매 응답 후 요약 모델을 사용하여 2~4개의 후속 제안을 생성하고 빠른 선택 버튼으로 표시합니다',
	...
	'settings.memoryConsolidateThreshold': '통합 임계값',
	'settings.memoryConsolidateThresholdDesc': '비중요 기억 풀이 이 항목 수를 초과하면 한 번의 LLM 통합이 실행되어 관련 항목을 병합하고 쓸모없는 항목을 삭제합니다. 0으로 설정하면 통합이 비활성화됩니다',
	'settings.memoryConsolidateCooldownHours': '통합 쿨다운 (시간)',
	'settings.memoryConsolidateCooldownHoursDesc': '두 번의 통합 실행 사이의 최소 간격 시간입니다. 연속 채팅 세션에서 토큰 낭비를 방지합니다',
	'settings.memoryConsolidation': '기억 통합',
	...
	// ── Custom agents ──────────────────────────────────────
	'settings.agentsSection': '에이전트',
	'settings.addAgent': '에이전트 추가',
	'settings.deleteAgentDesc': '이 에이전트를 목록에서 제거합니다',
	'settings.agentsEmpty': '사용자 지정 에이전트 없음',
	'settings.agentsEmptyDesc': '서브 에이전트를 추가하고 아래에서 도구 패턴, 모델, 설명을 직접 설정하세요',
	'settings.agentUntitled': '제목 없는 에이전트',
	...
	'settings.agentProfileDesc': '이 에이전트가 사용할 모델 설정. "상속"을 선택하면 런타임에 메인 에이전트의 모델을 사용합니다',
	'settings.agentProfileInherited': '상속',
	'settings.agentDescription': '설명',
	'settings.agentDescriptionDesc': '이 에이전트가 할 수 있는 일에 대한 짧은 고수준 설명. 메인 에이전트가 서브 에이전트에 위임할지 판단하는 데 사용됩니다. 구체적인 도구 이름은 나열하지 마세요',
	'settings.agentDescriptionPlaceholder': '예: 웹 검색을 수행하고 URL에서 실시간 콘텐츠를 가져옵니다',
	'settings.agentSystemPrompt': '시스템 프롬프트',
	'settings.agentSystemPromptDesc': '이 에이전트가 서브 에이전트로 실행될 때의 시스템 프롬프트입니다. 할당된 도구 사용 방법을 지시합니다. 비워두면 최소 기본값이 사용됩니다',
	'settings.agentSystemPromptPlaceholder': '예: 당신은 웹 검색 에이전트입니다. 광범위한 쿼리에는 web_search를, 전체 페이지 콘텐츠를 가져올 때는 web_fetch를 사용하세요…',
	'settings.agentToolsPreview': '도구 미리보기',
	'settings.agentToolsPreviewNone': '도구 패턴이 정의되지 않았습니다. 위에 패턴을 추가하면 일치하는 도구가 여기에 표시됩니다.',
	'settings.agentToolsPreviewNoMcp': '서버 도구가 구성되지 않았습니다. 먼저 설정 → Tools에서 서버를 설정하세요.',
	'settings.agentToolsPreviewNoMatch': '설정된 패턴과 일치하는 도구가 없습니다.',
	'settings.agentToolsPreviewCount': '{count}개의 도구 일치',
	'settings.agentGenerateDescriptionButton': '생성',
	'settings.agentGenerateDescriptionNoSummarizer': '요약 모델이 구성되지 않았습니다. 먼저 설정 → Profiles에서 지정하세요.',
	'settings.agentGenerateDescriptionNoTools': '사용 가능한 도구 정보가 없습니다.',
	'settings.agentGenerateDescriptionEmpty': '모델이 빈 결과를 반환했습니다. 다시 시도하세요.',
	'settings.agentBuiltinBadge': '내장',
	'settings.agentBuiltinDisabled': '비활성화됨',
	'settings.agentBuiltinToolsCount': '{count}개 도구 사용 가능',
	'settings.agentBuiltinToolsNone': '사용 가능한 도구가 없습니다.',
...
## 선택 영역 확장 [maximize-2]
다음 텍스트를 더 자세히, 예시나 배경을 추가해 확장하고 원래 의미를 유지해 주세요:
{{filepath}} {{selection}}
{{blockquote}}

원래 언어, 어조, 인라인 서식을 유지해 주세요. 앞뒤에 불필요한 줄바꿈을 추가하지 마세요. 결과를 노트에 직접 쓰고, 채팅에 수정된 본문을 답하지 마세요.

## 선택 영역 축약 [minimize-2]
다음 텍스트를 축약하되 모든 핵심 정보와 원래 의미를 유지해 주세요. 원래 언어, 어조, 인라인 서식을 유지해 주세요:
{{filepath}} {{selection}}
{{blockquote}}

결과를 노트에 직접 쓰고, 채팅에 수정된 본문을 답하지 마세요. 앞뒤에 불필요한 줄바꿈을 추가하지 마세요.

## 선택 영역 이어 쓰기 [chevrons-right]
지정된 텍스트 바로 다음에 이어서 써 주세요:
{{filepath}} {{selection}}
{{blockquote}}

결과를 노트의 지정 위치에 직접 쓰고, 채팅에 답하지 마세요. 앞뒤에 불필요한 줄바꿈을 추가하지 마세요.
...
## 글 다듬기 [wand-2]
다음 텍스트를 더 명확하고 간결하게 다듬고 원래 어조를 유지해 주세요:
{{filepath}} {{selection}}
{{blockquote}}
