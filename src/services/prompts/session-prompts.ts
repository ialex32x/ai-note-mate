...
- Picking the right edit tool for a single file:
    ...
    - Known line range to rewrite or delete → \`edit_lines\`. To insert content before a line → \`insert_lines_before\`.
...
When you have **multiple line-based edits to the same file** (e.g. fix a typo on line 12 AND rewrite a section on lines 40-50), submit them ALL in a single \`edit_lines\` call via its \`edits\` array. Do NOT call the tool multiple times — every edit's line numbers refer to the pre-edit file, and the tool applies them back-to-front so they don't interfere. Splitting into sequential calls uses stale line numbers and corrupts the file. Replacements (\`content != ""\`) and deletions (\`content == ""\`) are expressed as entries in the \`edits\` array — no \`op\` field needed. \`end_line\` is inclusive (range is \`[start_line, end_line]\`). To insert a new paragraph before a line, use a separate \`insert_lines_before\` call (before_line = the line to insert before; use totalLines + 1 to append). If you need both replace/delete AND insert operations on the same file, issue \`edit_lines\` first, then \`insert_lines_before\` (the \`edit_lines\` \`new_mtime\` can be chained into \`insert_lines_before\`'s \`expected_pre_edit_mtime\`).
