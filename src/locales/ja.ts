...
## 選択範囲を拡張 [maximize-2]
{{filepath}} の {{selection}} にある選択テキストを、詳細・例・背景を加えて拡張してください。原意、言語、トーン、インライン書式は保持してください。結果をノートに書き戻し、チャットに編集後の本文を返さないでください。
{{blockquote}}

## 選択範囲を短縮 [minimize-2]
{{filepath}} の {{selection}} にある選択テキストを短縮し、重要な事実と原意、言語、トーン、インライン書式は保持してください。結果をノートに書き戻し、チャットに編集後の本文を返さないでください。
{{blockquote}}

## 選択範囲から続きを書く [chevrons-right]
{{filepath}} の {{selection}} 選択範囲の末尾に続きを挿入してください。原文を繰り返したり言い換えたりしないでください。続きをノートに書き戻し、チャットに返さないでください。
{{blockquote}}
...
## 文章を改善 [wand-2]
{{filepath}} の {{selection}} にある選択テキストを、明確で簡潔になるよう推敲し、元のトーンは保ってください。結果をノートに書き戻し、チャットに編集後の本文を返さないでください。
{{blockquote}}
`,
...
ユーザーはまだ名前を入力しておらず、所在都市についても言及していません。`,
	...
	'tool.triggers.edit_lines':     '編集, 修正, 行を編集, 行を置換, 行を削除, ファイル修正, ファイル更新',
	'tool.triggers.insert_lines_before': '行を挿入, 前に挿入, 行を追加, 段落を挿入, 新しい行を追加, 行を挿入する',
