...
// ─── Provider mocks ────────────────────────────────────────
//
// `ContextReducer.reduce` calls `summarizeConversation` →
...
import { ContextReducer, HistroyMessage, ConversationSummary } from '../src/services/context-reducer';
...
// contract.
describe('ContextReducer.reduce — Level-2 merge', () => {
    it('keeps the current user turn and the recent raw window when Level-2 triggers', async () => {
        ...
        const result = await ContextReducer.reduce(
            MODEL_CONFIG,
            PROMPT,
            rawMessages,
            summaries,
    ...
    it('converges: applying the replacement collapses the summary count and stops re-triggering Level-2', async () => {
        ...
        const r1 = await ContextReducer.reduce(
        ...
        const r2 = await ContextReducer.reduce(
            MODEL_CONFIG, PROMPT, rawMessages, summaries, { compressionThreshold: 10_000_000 },
        );
