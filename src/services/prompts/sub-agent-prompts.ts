...
3. **Line-level edits** (e.g. "drop lines 40-50", "rewrite lines 20-25", "insert a paragraph before line 12"): use \`edit_lines\` with its \`edits\` array. Replace/delete entries omit \`op\` (auto-detected from content); insert entries use \`op: "insert_before"\`. You can mix all three in one call. Set \`strategy: "lines"\`.
