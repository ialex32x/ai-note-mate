...
export function createWebAgentPrompt(): string {
...
- A \`web_fetch_url\` failure (HTTP error, anti-bot challenge, empty content, "no readable text"…) means
  ...
  with minor variations, and do NOT chain through many other URLs hoping one will work. Either fall back to
  \`web_search\` for a different source, or report the failure to the caller honestly.
...
1. **Wholesale rewrite** (reformat / translate / restructure the whole note): call \`write_file\` with the new full body. \`write_file\` is the ONLY tool that performs whole-file overwrite — \`create_note\` strictly creates NEW notes and the main agent does not have \`write_file\` at all, which is exactly why this task was delegated to you. Pass \`expected_pre_edit_mtime\` equal to the \`mtime\` you got from \`read_file\` / \`read_section\` / \`get_metadata\` / \`get_file_state\`, so a concurrent external edit is caught. Do NOT pass any size value as a race guard — character count and on-disk byte count differ on CRLF / multi-byte / BOM files and would yield false-positive race errors. Set \`strategy: "wholesale"\` in your result.
2. **Surgical multi-region edits** (handful of typos, heading renames, paragraph-sized rewrites in known locations): call \`batch_replace_text\` ONCE with all regions in its \`replacements\` array (batching is atomic — splitting into multiple \`replace_text\` calls corrupts offsets). For whole-section replacement, use \`set_section\` with a \`body_hash\` from a prior \`read_section\`. Set \`strategy: "surgical"\`.
