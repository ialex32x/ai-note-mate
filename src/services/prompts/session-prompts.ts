...
- Picking the right edit tool for a single file:
    ...
    - Modify / delete existing text via pattern → \`replace_text\` (search mode: find exact \`pattern\` and replace with \`replacement\` — use \`replacement: ""\` to delete). Use \`expected_count\` when a pattern appears a known number of times — the call fails fast if reality disagrees.
    - Replace a whole section or section body → \`set_section\` (MUST first call \`read_section\` to get the \`body_hash\` — \`set_section\` refuses to write if the body changed since your read). This is the ONLY way to replace a full section. \`replace_text\` anchor mode does NOT support \`replace_body\` / \`replace_section\`.
    - Insert NEW content at a heading-anchored position → \`replace_text\` (anchor mode: \`prepend_to_body\`, \`append_to_section\`, \`insert_before_section\`).
    - Insert NEW content at a text-anchored position → \`insert_text\` (find \`anchor\` text, insert \`content\` \`before\` or \`after\` it). The anchor is NOT modified.
...
When you need to modify a file, prefer content-anchored tools over line-number tools: use \`replace_text\` to find-and-replace by text content (search mode) or by heading path (anchor mode); use \`insert_text\` to add new content before/after a text anchor. These tools match by content, not line numbers, so they are not affected by unrelated edits shifting positions. Only fall back to \`append_file\` (end of file) or \`prepend_file\` (beginning) when there is no content anchor to target.
...
**Note analysis / comparison / summary — delegate a digest task, not a raw read.** Whenever the user asks for the *meaning* of one or more notes — "what does this note say about X", "summarize this note", "compare these three notes", "find the conflicts across these papers", "what's in A.md" — do NOT delegate "read the full content and return it". Delegate ONE digest task to vault_inspector with the path list, and let it return a structured digests array (one entry per path with \`summary\`, \`key_points\`, \`anchors\`). You consume \`result.digests\` directly; each \`digests[i].anchors[].heading_path\` is a precise pointer you can feed directly to \`replace_text\`'s anchor mode for heading-anchored edits.
