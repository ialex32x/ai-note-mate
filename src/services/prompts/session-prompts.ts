...
- Picking the right edit tool for a single file:
    ...
    - Modify / delete existing text → \`replace_text\` (search mode: find exact \`pattern\` and replace with \`replacement\` — use \`replacement: ""\` to delete; anchor mode: navigate by \`heading_path\` + \`where\` to replace a whole section or its body). Use \`expected_count\` when a pattern appears a known number of times — the call fails fast if reality disagrees.
    - Insert NEW content at a text-anchored position → \`insert_text\` (find \`anchor\` text, insert \`content\` \`before\` or \`after\` it). The anchor is NOT modified. For heading-anchored insertion (\`prepend_to_body\`, \`append_to_section\`, \`insert_before_section\`), use \`replace_text\` with anchor mode instead.
