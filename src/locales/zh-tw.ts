export const zhTW: Record<string, string> = {
	...
	'settings.speechToTextConfig': '語音轉文字',
	'settings.speechToTextConfigDesc': '選擇已儲存的語音轉文字配置',
	'settings.addSpeechToTextConfig': '新增配置',
	'settings.sttConfigName': '配置名稱',
	'settings.sttConfigNamePlaceholder': '為此配置輸入名稱',
	'settings.deleteSpeechToTextConfigDesc': '移除此配置。至少保留一個配置',
	'settings.sttApiKeyDesc': '用於語音轉文字的 API 金鑰',
	'settings.sttRegion': '區域',
	'settings.sttRegionDesc': '選擇 DashScope 語音轉文字服務的區域',
	'settings.sttRegionCnBeijing': '華北2（北京）',
	'settings.sttRegionUsEast1': '美國（維吉尼亞）',
	'settings.sttRegionApSoutheast1': '新加坡',
	'settings.sttRegionEuCentral1': '德國（法蘭克福）',
	'settings.sttWorkspaceId': 'Workspace ID',
	'settings.sttWorkspaceIdDesc': 'DashScope 工作空間 ID。新加坡和法蘭克福區域必填',
	'settings.sttShortModel': '短音訊模型',
	'settings.sttShortModelDesc': '用於短音訊即時轉寫的模型',
	'settings.sttLongModel': '長音訊模型',
	'settings.sttLongModelDesc': '用於長音訊非同步檔案轉寫的模型',
	'settings.sttEmpty': '尚未配置語音轉文字。新增一個以啟用音頻轉錄功能。',
	'settings.tcSecretId': 'SecretId',
	'settings.tcSecretIdDesc': '騰訊雲 SecretId。請在騰訊雲控制台取得',
	'settings.tcSecretKey': 'SecretKey',
	'settings.tcSecretKeyDesc': '騰訊雲 SecretKey。請在騰訊雲控制台取得',
	'settings.tcAsrRegion': 'ASR 地域',
	'settings.tcAsrRegionDesc': '騰訊雲語音辨識服務地域（例如 ap-guangzhou、ap-shanghai）',
	'settings.tcEngineModelType': '引擎模型',
	'settings.tcEngineModelTypeDesc': '語音辨識引擎類型。16k_zh 為中文通用，8k_zh 為電話場景',
	'settings.tcCosBucket': 'COS 儲存貯體（選用）',
	'settings.tcCosBucketDesc': 'COS 儲存貯體名稱（格式：name-appid）。大於 5 MB 的檔案需要',
	'settings.tcCosRegion': 'COS 地域',
	'settings.tcCosRegionDesc': 'COS 儲存貯體地域（例如 ap-guangzhou）。設定儲存貯體後必須填寫',
	...
	'settings.agentMdTemplate': '## 規則\n- 使用清晰、簡潔的語言。推薦使用 Markdown 格式化。\n- 提及相關筆記時，使用維基連結（wiki-link）。\n- 當請求不明確時，主動追問澄清。\n\n## Mermaid 圖表\n當用戶的問題涉及需要透過視覺化來呈現的資料（比較、佔比、趨勢、流程）時，請輸出 Mermaid 程式碼區塊。使用：\n- `pie` 表示佔比 / 百分比\n- `xychart-beta` 表示長條圖 / 比較\n- `flowchart` 或 `graph` 表示流程 / 關係\n- `sequenceDiagram` 表示互動 / 時間線\n始終優先使用真實資料，不要憑空捏造數字。',
	...
	'settings.builtinCodeAgent': '內建程式碼智慧體',
	'settings.builtinCodeAgentDesc': '啟用內建的程式碼智慧體，為助手提供 JavaScript 執行工具，使其在需要時可執行小段程式碼',
	'settings.followUpSection': 'Follow-up 相關',
	'settings.followUpSuggestions': '追問建議按鈕',
	'settings.followUpSuggestionsDesc': '每次回覆後，使用摘要模型生成 2–4 條追問建議，以快捷按鈕形式展示',
	...
	'settings.artifactStoreDesc': '保存過大的工具與子智慧體回傳值（如 delegate 結果、上下文壓縮時 spill 的 envelope 欄位），避免對話壓縮後遺失；主智慧體可透過 recall_artifact 按需取回。依會話隔離；活躍項目受下列上限約束，並寫入該會話目錄，重載外掛後仍可恢復',
	...
	'settings.subAgentFilterTopK': '子智慧體檢索:Top-K',
	'settings.subAgentFilterTopKDesc': '每輪注入到 DELEGATION 段和 delegate_task 工具列舉中的子智慧體上限 (1–8)。沿用與工具過濾相同的 BM25 + 嵌入混合檢索器。設為較低值可在不需要子智慧體的對話輪中節省 token;本會話中曾經呼叫過的子智慧體仍會保留(歷史黏性)。設為 0 表示使用內建預設值',
	...
	'settings.memoryConsolidateThreshold': '整理觸發閾值',
	'settings.memoryConsolidateThresholdDesc': '當非關鍵記憶條目數超過此閾值時，觸發一次 LLM 整理，合併相關條目並刪除過時內容，保持記憶池精簡。設為 0 則停用整理',
	'settings.memoryConsolidateCooldownHours': '整理冷卻時間（小時）',
	'settings.memoryConsolidateCooldownHoursDesc': '兩次整理操作之間的最小間隔小時數。避免在高頻對話中重複消耗 tokens',
	'settings.memoryConsolidation': '記憶整理',
	...
	// ── Custom agents ──────────────────────────────────────
	'settings.agentsSection': '智慧體',
	'settings.addAgent': '新增智慧體',
	'settings.deleteAgentDesc': '從清單中移除此智慧體',
	'settings.agentsEmpty': '尚無自訂智慧體',
	'settings.agentsEmptyDesc': '新增一個子智慧體，在下方直接設定其工具模式、模型和描述',
	'settings.agentUntitled': '未命名智慧體',
	...
	'settings.agentProfileDesc': '此智慧體使用的模型設定。選擇「繼承」則在執行時使用主智慧體的模型',
	'settings.agentProfileInherited': '繼承',
	'settings.agentDescription': '描述',
	'settings.agentDescriptionDesc': '此智慧體能力的簡短高層描述。主智慧體會根據此描述判斷是否將任務委派給它。不要列舉具體的工具名稱',
	'settings.agentDescriptionPlaceholder': '例如：可以搜尋網路並取得網頁即時內容',
	'settings.agentSystemPrompt': '系統提示詞',
	'settings.agentSystemPromptDesc': '該智慧體作為子智慧體執行時的系統提示詞。告知智慧體如何使用其分配的工具。留空則使用最小預設值',
	'settings.agentSystemPromptPlaceholder': '例如：你是一個網頁搜尋智慧體。使用 web_search 進行廣泛查詢，使用 web_fetch 取得完整頁面內容……',
	'settings.agentToolsPreview': '工具預覽',
	'settings.agentToolsPreviewNone': '未定義工具比對模式，請在上方新增模式以檢視符合的工具。',
	'settings.agentToolsPreviewNoMcp': '尚未設定伺服器工具，請先至 設定 → Tools 設定伺服器。',
	'settings.agentToolsPreviewNoMatch': '沒有工具符合目前設定的模式。',
	'settings.agentToolsPreviewCount': '符合 {count} 個工具',
	'settings.agentGenerateDescriptionButton': '生成',
	'settings.agentGenerateDescriptionNoSummarizer': '尚未設定摘要模型。請先在 設定 → Profiles 中指定。',
	'settings.agentGenerateDescriptionNoTools': '沒有可用的工具資訊。',
	'settings.agentGenerateDescriptionEmpty': '模型回傳了空結果，請重試。',
	'settings.agentBuiltinBadge': '內建',
	'settings.agentBuiltinDisabled': '已停用',
	'settings.agentBuiltinToolsCount': '{count} 個可用工具',
	'settings.agentBuiltinToolsNone': '無可用工具。',
...
`,
	...
	'status.checking': '檢查中…',
	'status.ok': '正常',
	'status.errorLabel': '錯誤',
	'status.apiKeyRequired': '需要 API 金鑰。',
...
使用者還沒有填寫名字, 也沒有提及所在城市.`,
	...
	// 文案隨 UI 語言在地化；結尾的說明會額外要求模型
	// 使用對話當前所用的語言回覆，以便對話語言
	// 偏離 UI 語言時也能對齊。
