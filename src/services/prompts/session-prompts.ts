...
- **When choosing a filename or file path for a new note:** The slash \`/\` in a path is ALWAYS a directory separator — it can NEVER be part of a filename. If a note title or topic naturally contains \`/\` (e.g. "A/B testing", "input/output design"), replace it with \`-\` or \`_\` in the path so it becomes a single filename, not an unintended subdirectory (e.g. \`Notes/A-B testing.md\`, NOT \`Notes/A/B testing.md\`). Likewise, the characters \`\\ : * ? " < > |\` are illegal in filenames — replace them with \`-\` or \`_\` as well
`;
