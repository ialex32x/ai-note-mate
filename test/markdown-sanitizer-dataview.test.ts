...
//
// Strategy: strip ALL dataview/dataviewjs blocks during streaming (whether
// the closing fence has arrived or not).  Even a complete block must not be
// evaluated mid-stream because the Dataview plugin executes the query and
// the content may still change, producing flickering results or errors.
// The final, clean render happens in finalize() once streaming is complete.
// ─────────────────────────────────────────────────────────────────────────────
...
describe("deferTrailingDataviewBlock (via sanitizeStreamingMarkdown)", () => {
    ...
    // Even complete blocks are stripped during streaming because Dataview
    // evaluates the query immediately, and the query content may still change.

    it("strips a complete dataview block (closing fence present)", () => {
        const input = "Before\n\n```dataview\nLIST\nFROM #tag\nWHERE file.ctime > date(today) - dur(7 days)\n```\n\nAfter";
        const result = sanitize(input);
        expect(result).not.toContain("dataview");
        expect(result).not.toContain("LIST");
        expect(result).not.toContain("#tag");
        // Non-dataview content is preserved
        expect(result).toContain("Before");
        expect(result).toContain("After");
    });

    it("strips a complete dataview block followed by another code block", () => {
        const input =
            "```dataview\nTABLE file.ctime\n```\n\n```python\nprint('hello')\n```";
        const result = sanitize(input);
        expect(result).not.toContain("dataview");
        expect(result).not.toContain("TABLE");
        // Non-dataview code block is preserved
    ...
    it("streaming: strips throughout (including after closing fence arrives)", () => {
        ...
        // Step 4: block complete — still stripped; finalize() renders it.
        const s4 = sanitize(
            "Intro\n\n```dataview\nTABLE\nFROM #tags\nWHERE file.ctime > date(today) - dur(7 days)\n```\n\nConclusion",
        );
        expect(s4).not.toContain("dataview");
        expect(s4).not.toContain("TABLE");
        expect(s4).not.toContain("FROM #tags");
        // Non-dataview content preserved
        expect(s4).toContain("Intro");
    ...
    it("strips complete dataview block when opener has space: ``` dataview ... ```", () => {
        const input = "Text\n\n``` dataview\nLIST FROM #project\n```\n\nAfter";
        const result = sanitize(input);
        expect(result).not.toContain("dataview");
        expect(result).not.toContain("LIST");
    ...
    it("strips a complete dataviewjs block when closed", () => {
        const input = "```dataviewjs\ndv.table(['Name'], dv.pages().map(p => [p.file.name]))\n```\n\nDone";
        const result = sanitize(input);
        expect(result).not.toContain("dataviewjs");
        expect(result).not.toContain("dv.table");
        // Non-dataviewjs content preserved
        expect(result).toContain("Done");
    });

    // ── Interaction with mermaid deferral ──────────────────────────────────

    it("keeps a complete mermaid block but strips trailing dataview", () => {
        const input =
            "```mermaid\ngraph TD\nA --> B\n```\n\nMid\n\n```dataview\nTABLE\n";
        const result = sanitize(input);
        // Mermaid block is complete → kept (mermaid only strips unclosed blocks)
        expect(result).toContain("mermaid");
        expect(result).toContain("graph TD");
        expect(result).toContain("Mid");
        // Dataview block is unclosed and trailing → stripped
        expect(result).not.toContain("dataview");
        expect(result).not.toContain("TABLE");
    });
    ...
    it("defers both unclosed mermaid and dataview blocks", () => {
        const input =
            "Intro\n\n```mermaid\ngraph TD\n\n\n```dataview\nTABLE\n";
        const result = sanitize(input);
        // Both are unclosed → both stripped
    ...
    it("handles closing fence with trailing whitespace", () => {
        const input = "```dataview\nTABLE file.name\n```   \n\nDone";
        const result = sanitize(input);
        // Dataview block stripped during streaming even with trailing whitespace
        expect(result).not.toContain("dataview");
        expect(result).not.toContain("TABLE");
        expect(result).toContain("Done");
    });
    ...
    it("does NOT strip unclosed block named similarly to dataview (e.g. dataview-example)", () => {
        ...
        const result = sanitize(input);
        // The regex matches "dataview" prefix → block is stripped
    ...
    it("strips all dataview blocks (both complete first block and unclosed trailing second)", () => {
        const input =
            "```dataview\nTABLE file.name\nFROM #todo\n```\n\nMid\n\n```dataviewjs\ndv.list(\n";
        const result = sanitize(input);
        // Both blocks are stripped during streaming
        expect(result).not.toContain("TABLE");
