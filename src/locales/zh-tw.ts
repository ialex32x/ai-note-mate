export const zhTW: Record<string, string> = {
	...
	'settings.insightsProfile': '見解萃取設定檔',
	'settings.insightsProfileDesc': '用於萃取見解的 Provider 設定檔。若需要比摘要器更強的結構化輸出，可指定獨立設定檔；預設與摘要器相同。',
	'settings.insightsProfileSameAsSummarizer': '與摘要器相同',
...
`,
	...
	'tips.configureProfile.title': '設定 AI 方案',
	'tips.configureProfile.body': '開始對話前，請在目前使用的方案中填寫 Base URL、模型與 API 金鑰。API 金鑰透過 Obsidian Keychain 儲存，不會以明文寫入外掛設定，可放心使用。點擊「試試看」可直接跳轉到方案分區。',
	'tips.configureProfile.openFailed': '無法自動開啟設定面板，請手動前往 設定 → Note Mate → 方案。',
